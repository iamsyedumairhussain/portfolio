...
const Form = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const sentCount = Number(sessionStorage.getItem("emailCount")) || 0;

    if (sentCount >= 2) {
      alert("You can only send 2 messages per session.");
      return;
    }
    ...
    emailjs
      .sendForm("service_8of0hoi", "template_1zs797f", event.target, {
        publicKey: "EOyG18yRSnKkeDq6B",
      })
      .then(
        () => {
          sessionStorage.setItem("emailCount", sentCount + 1);
